...
{
    SFR_nERROR enErrorReg;
    enErrorReg = SFR__enStatusInterruptSourceByNumber(enInterruptArg, penStatusArg);
    return (enErrorReg);
}
...
SFR_nERROR SFR__enStatusMaskedInterruptSourceByMask(UBase_t uxInterruptMaskArg, UBase_t* puxInterruptStatusArg)
{
    ...
    if(SFR_enERROR_OK == enErrorReg)
    {
        enErrorReg = SFR__enGetInterruptSourceStateByMask(uxInterruptMaskArg, &uxInterruptMaskState);
    }
    if(SFR_enERROR_OK == enErrorReg)
    {
        enErrorReg = SFR__enStatusInterruptSourceByMask(uxInterruptMaskArg, &uxInterruptMaskStatus);
    }
    if(SFR_enERROR_OK == enErrorReg)
    {
...
SFR_nERROR SFR__enStatusMaskedInterruptSourceByNumber(SFR_nINTERRUPT enInterruptArg, SFR_nSTATUS* penStatusArg)
{
    ...
    if(SFR_enERROR_OK == enErrorReg)
    {
        enErrorReg = SFR__enGetInterruptSourceStateByNumber(enInterruptArg, &enStateReg);
    }
    if(SFR_enERROR_OK == enErrorReg)
    {
        enErrorReg = SFR__enStatusInterruptSourceByNumber(enInterruptArg, &enStatusReg);
    }
    if(SFR_enERROR_OK == enErrorReg)
    {
...
{
    SFR_nERROR enErrorReg;
    enErrorReg = SFR__enStatusMaskedInterruptSourceByNumber(enInterruptArg, penStatusArg);
    return (enErrorReg);
}
