 ...
 */
#include <xDriver_MCU/AES/Driver/Intrinsics/Primitives/xHeader/AES_WriteRegister.h>
...
AES_nERROR AES__enWriteRegister8Bits(AES_Register8Bits_t* pstRegisterDataArg)
{
    ...
    if(AES_enERROR_OK == enErrorReg)
    {
        ...
        enErrorReg = (AES_nERROR) MCU__enWriteRegister8Bits(pstRegisterDataArg);
    }
    return (enErrorReg);
}
...
AES_nERROR AES__enWriteRegister(AES_Register_t* pstRegisterDataArg)
{
    ...
    if(AES_enERROR_OK == enErrorReg)
    {
        uptrModuleBase = AES__uptrBlockBaseAddress();
        pstRegisterDataArg->uptrAddress += uptrModuleBase;
        enErrorReg = (AES_nERROR) MCU__enWriteRegister(pstRegisterDataArg);
    }
    return (enErrorReg);
}
